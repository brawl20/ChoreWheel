...
void MainLoopStep()
{
    ...
    else Application.buildWindow(&show_demo_window);
...
JNIEXPORT jstring JNICALL Java_com_example_tcpclient_MainActivity_connectToServer(JNIEnv *env, jobject thiz, jstring serverAddress, jint port)
{
    const char *serverAddr = env->GetStringUTFChars(serverAddress, 0);
    ...
    if (inet_aton(serverAddr, &server_addr.sin_addr) == 0) {
        ...
        close(sockfd);
        return env->NewStringUTF("Invalid IP address");
    }
